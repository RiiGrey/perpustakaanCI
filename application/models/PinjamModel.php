...
class PinjamModel extends CI_Model{
    var $table = 'tbpinjam'; //nama tabel dari database
	var $column_order = array(null, 'NoAnggota', 'Nama','KodeBuku','Judul', 'Pinjam', 'Kembali'); //field yang ada di table user
	var $column_search = array('NoAnggota', 'Nama','KodeBuku','Judul', 'Pinjam', 'Kembali'); //field yang diizin untuk pencarian 
	...
	public function fetchBuku(){
		$query = $this->db->query('SELECT * FROM tbbuku ORDER BY Judul');

		return $query;
	}
	
	public function fetchAnggota(){
		$query = $this->db->query('SELECT * FROM tbanggota ORDER BY Nama');
